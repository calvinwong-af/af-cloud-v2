"""
routers/pricing/dg_class_charges.py — DG Class Charges rate endpoints (two-tier schema).

Tables:
  dg_class_charge_cards  — card identity (port, direction, type, dg_class, charge_code, etc.)
  dg_class_charges       — rate rows (price, cost, effective_from/to) with rate_card_id FK
"""
...
_VALID_UOMS = {"CONTAINER", "CBM", "KG", "W/M", "SET", "BL"}
_VALID_DG_CLASS_CODES = {"DG-2", "DG-3"}
...
class DgClassChargeCardUpdate(BaseModel):
...
class DgClassChargeCreate(BaseModel):
    # Card identity
    port_code: str
    trade_direction: str
...
class DgClassChargeRateUpdate(BaseModel):
...
def _card_to_dict(r) -> dict:
    return {
        ...
        "dg_class_code": r[5], "container_size": r[6], "container_type": r[7],
...
def _build_card_key(port_code, trade_direction, shipment_type, dg_class_code,
                    container_size, container_type, charge_code, is_domestic, is_international) -> str:
    return (f"{port_code}|{trade_direction}|{shipment_type}|{dg_class_code}|"
            f"{container_size}|{container_type}|{charge_code}|"
            f"{str(is_domestic).lower()}|{str(is_international).lower()}")
...
@router.get("/ports")
async def list_dg_class_charge_ports(
    country_code: Optional[str] = Query(default=None),
    claims: Claims = Depends(require_afu),
    conn=Depends(get_db),
):
    """Return distinct port_code values from dg_class_charge_cards, optionally filtered by country."""
    params: dict = {}
    ...
    rows = conn.execute(text(f"""
        SELECT DISTINCT dcc.port_code
        FROM dg_class_charge_cards dcc
        JOIN ports p ON p.un_code = dcc.port_code
        WHERE {where}
        ORDER BY dcc.port_code
    """), params).fetchall()
...
@router.get("/cards")
async def list_dg_class_charge_cards(
    port_code: str = Query(...),
    ...
    """Return DG class charge cards with time_series built from rate rows."""
    if not port_code:
    ...
    card_rows = conn.execute(text(f"""
        ...
        ORDER BY trade_direction, shipment_type, dg_class_code, container_size, container_type, charge_code
    """), params).fetchall()
...
async def get_dg_class_charge(
    ...
    row = conn.execute(text("""
        SELECT dc.id, dc.rate_card_id, dc.price, dc.cost,
               dc.effective_from, dc.effective_to, dc.created_at, dc.updated_at,
               dcc.port_code, dcc.trade_direction, dcc.shipment_type,
               dcc.dg_class_code, dcc.container_size, dcc.container_type,
               dcc.charge_code, dcc.description, dcc.currency, dcc.uom,
               dcc.is_domestic, dcc.is_international, dcc.is_active
        FROM dg_class_charges dc
        JOIN dg_class_charge_cards dcc ON dcc.id = dc.rate_card_id
        WHERE dc.id = :id
    """), {"id": rate_id}).fetchone()

    if not row:
        raise HTTPException(status_code=404, detail="DG class charge not found")
    ...
    return {"status": "OK", "data": {
        ...
        "dg_class_code": row[11],
        "container_size": row[12],
        "container_type": row[13],
        "charge_code": row[14],
        "description": row[15],
        "currency": row[16],
        "uom": row[17],
...
@router.post("/rates")
async def create_dg_class_charge(
    body: DgClassChargeCreate,
    ...
    card_key = _build_card_key(
        ...
        body.dg_class_code, body.container_size, body.container_type,
        body.charge_code, body.is_domestic, body.is_international,
    )

    # Find or create card
    card_row = conn.execute(text(
        "SELECT id FROM dg_class_charge_cards WHERE rate_card_key = :key"
    ...
    if card_row:
        ...
        """), {
            ...
            "stype": body.shipment_type, "dg_class": body.dg_class_code,
            "csize": body.container_size, "ctype": body.container_type,
    ...
    existing = conn.execute(text("""
        SELECT id FROM dg_class_charges
        WHERE rate_card_id = :card_id AND effective_from = :eff_from
    """), {"card_id": card_id, "eff_from": body.effective_from}).fetchone()

    if existing:
        raise HTTPException(status_code=409, detail="Duplicate DG class charge entry")

    # Insert rate row
    result = conn.execute(text("""
        INSERT INTO dg_class_charges (rate_card_id, price, cost,
...
async def update_dg_class_charge_card(
    card_id: int,
    body: DgClassChargeCardUpdate,
    claims: Claims = Depends(require_afu_admin),
    conn=Depends(get_db),
):
    """Update card-level fields on a dg_class_charge_cards row."""
    existing = conn.execute(text(
        "SELECT id, port_code, trade_direction, shipment_type, dg_class_code, "
        "container_size, container_type, charge_code, is_domestic, is_international "
        "FROM dg_class_charge_cards WHERE id = :id"
    ), {"id": card_id}).fetchone()
    if not existing:
        raise HTTPException(status_code=404, detail="DG class charge card not found")
    ...
    if provided & _KEY_FIELDS:
        key_vals = {
            ...
            "dg_class_code": params.get("dg_class_code", existing[4]),
            "container_size": params.get("container_size", existing[5]),
            "container_type": params.get("container_type", existing[6]),
    ...
    conn.execute(text(
        f"UPDATE dg_class_charge_cards SET {', '.join(updates)} WHERE id = :id"
    ), params)
    conn.commit()
...
async def update_dg_class_charge_rate(
    rate_id: int,
    body: DgClassChargeRateUpdate,
    claims: Claims = Depends(require_afu_admin),
    conn=Depends(get_db),
):
    """Update rate-level fields on a dg_class_charges row."""
    existing = conn.execute(text(
        "SELECT id FROM dg_class_charges WHERE id = :id"
    ), {"id": rate_id}).fetchone()
    if not existing:
        raise HTTPException(status_code=404, detail="DG class charge not found")
    ...
    if "effective_from" not in provided or "effective_to" not in provided:
        row = conn.execute(text(
            "SELECT effective_from, effective_to FROM dg_class_charges WHERE id = :id"
        ), {"id": rate_id}).fetchone()
        if row:
    ...
    conn.execute(text(
        f"UPDATE dg_class_charges SET {', '.join(updates)} WHERE id = :id"
    ), params)
    conn.commit()
...
async def delete_dg_class_charge(
    rate_id: int,
    claims: Claims = Depends(require_afu_admin),
    conn=Depends(get_db),
):
    existing = conn.execute(text("SELECT id FROM dg_class_charges WHERE id = :id"), {"id": rate_id}).fetchone()
    if not existing:
        raise HTTPException(status_code=404, detail="DG class charge not found")

    conn.execute(text("DELETE FROM dg_class_charges WHERE id = :id"), {"id": rate_id})
    conn.commit()
...
async def delete_dg_class_charge_card(
    card_key: str,
    claims: Claims = Depends(require_afu_admin),
    conn=Depends(get_db),
):
    ...
    result = conn.execute(text(
        "DELETE FROM dg_class_charges WHERE rate_card_id = :card_id"
    ), {"card_id": card_id})
    rate_count = result.rowcount

    # Delete the card row
    conn.execute(text(
        "DELETE FROM dg_class_charge_cards WHERE id = :card_id"
    ), {"card_id": card_id})
    conn.commit()
