"""
routers/pricing/thc.py — THC (Terminal Handling Charges) rate endpoints.
"""
...
def _row_to_dict(r) -> dict:
    return {
        ...
        "currency": r[8],
        "uom": r[9],
        "effective_from": str(r[10]) if r[10] else None,
...
_SELECT = """
    SELECT id, port_code, trade_direction, shipment_type, equipment_type,
...
async def list_thc_rates(
    ...
    if port_code:
        where.append("port_code = :port_code")
    ...
    rows = conn.execute(text(f"""
        ...
        ORDER BY port_code, trade_direction, shipment_type, equipment_type, charge_code
    """), params).fetchall()
...
async def get_thc_rate(
    ...
    if not row:
        raise HTTPException(status_code=404, detail="THC rate not found")

    return {"status": "OK", "data": _row_to_dict(row)}


@router.post("/rates")
async def create_thc_rate(
    body: THCRateCreate,
    ...
    existing = conn.execute(text("""
        SELECT id FROM thc_rates
        WHERE port_code = :port_code AND trade_direction = :direction
          AND shipment_type = :stype AND equipment_type IS NOT DISTINCT FROM :etype
          AND charge_code = :charge AND effective_from = :eff_from
    """), {
    ...
    if existing:
        raise HTTPException(status_code=409, detail="Duplicate THC rate entry")

    now = datetime.now(timezone.utc).isoformat()
    result = conn.execute(text("""
        INSERT INTO thc_rates (port_code, trade_direction, shipment_type, equipment_type,
                               charge_code, description, amount, currency, uom,
                               effective_from, effective_to, is_active, created_at, updated_at)
        VALUES (:port_code, :direction, :stype, :etype,
...
async def update_thc_rate(
    rate_id: int,
    body: THCRateUpdate,
    claims: Claims = Depends(require_afu_admin),
    conn=Depends(get_db),
):
    existing = conn.execute(text("SELECT id FROM thc_rates WHERE id = :id"), {"id": rate_id}).fetchone()
    if not existing:
        raise HTTPException(status_code=404, detail="THC rate not found")
    ...
    conn.execute(text(f"UPDATE thc_rates SET {', '.join(updates)} WHERE id = :id"), params)
    conn.commit()
...
async def delete_thc_rate(
    ...
    conn.execute(text("DELETE FROM thc_rates WHERE id = :id"), {"id": rate_id})
    conn.commit()
