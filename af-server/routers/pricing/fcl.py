...
async def list_fcl_rate_cards(
    ...
    if alerts_only:
        where.append("""(
            ...
            OR
            (
                EXISTS (SELECT 1 FROM fcl_rates r WHERE r.rate_card_id = rc.id AND r.supplier_id IS NULL AND r.rate_status = 'PUBLISHED' AND r.effective_from <= CURRENT_DATE AND (r.effective_to IS NULL OR r.effective_to >= CURRENT_DATE) AND r.list_price IS NOT NULL)
                AND NOT EXISTS (SELECT 1 FROM fcl_rates r WHERE r.rate_card_id = rc.id AND r.supplier_id IS NOT NULL AND r.rate_status = 'PUBLISHED' AND r.effective_from <= CURRENT_DATE AND (r.effective_to IS NULL OR r.effective_to >= CURRENT_DATE) AND r.cost IS NOT NULL)
            )
        )""")
...
async def update_fcl_rate(
    ...
    if "effective_from" in body.__fields_set__ or "effective_to" in body.__fields_set__:
        ...
        if "effective_from" not in body.__fields_set__ or "effective_to" not in body.__fields_set__:
            row = conn.execute(text("SELECT effective_from, effective_to FROM fcl_rates WHERE id = :id"), {"id": rate_id}).fetchone()
            if row:
