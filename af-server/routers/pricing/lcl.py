...
class LCLResolveRequest(BaseModel):
    quantity: float          # W/M quantity (CBM or tonne, whichever is greater)
    supplier_id: Optional[str] = None
...
async def resolve_lcl_rate(
    card_id: int,
    body: LCLResolveRequest,
    claims: Claims = Depends(require_afu),
    conn=Depends(get_db),
):
    if body.quantity <= 0:
    ...
    roundup = rate.get("roundup_qty") or 0
    if roundup > 0:
        import math
        billable_qty = math.ceil(qty / roundup) * roundup
    else:
        billable_qty = qty

    # Select the correct price field based on supplier_id
    if body.supplier_id is None:
        rate_per_unit = rate.get("list_price")
        min_quantity = rate.get("min_quantity")
    else:
        rate_per_unit = rate.get("cost")
        min_quantity = rate.get("min_quantity")

    if rate_per_unit is None:
        raise HTTPException(status_code=404, detail="No rate value available")

    surcharges = rate.get("surcharges") or []
    surcharge_per_unit = _surcharge_total(surcharges)

    # Apply minimum quantity floor
    effective_qty = max(billable_qty, min_quantity) if min_quantity is not None else billable_qty
    min_applied = min_quantity is not None and billable_qty < min_quantity

    base_charge = effective_qty * rate_per_unit
    surcharge_amount = effective_qty * surcharge_per_unit
    ...
    return {"status": "OK", "data": {
        ...
        "roundup_qty": roundup,
        "surcharge_total_per_unit": surcharge_per_unit,
