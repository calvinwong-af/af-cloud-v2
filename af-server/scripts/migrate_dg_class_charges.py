"""
scripts/migrate_dg_class_charges.py

Migrate DG class charges data from Google Cloud Datastore to PostgreSQL.

Reads:
  - PricingDGClassCharges — charge line item definitions
  - PTMonthlyRatePortCharges — monthly rate history (kind=PT-DGC-CHARGES)

Usage (from project root C:\\dev\\af-cloud-v2):
    af-server\\.venv\\Scripts\\python.exe af-server/scripts/migrate_dg_class_charges.py
"""
...
# Narrower than local_charges — DG charges don't use CW_KG, QTL, RAIL_3KG
VALID_UOMS = {"CONTAINER", "CBM", "KG", "W/M", "SET", "BL"}

UOM_REMAP = {
    "CTR": "CONTAINER",
    "RT": "W/M",
    "CW": "CW_KG",      # will fail VALID_UOMS check and be skipped
    "C3KG": "RAIL_3KG", # will fail VALID_UOMS check and be skipped
}
...
VALID_DG_CLASS_CODES = {"DG-2", "DG-3"}  # no ALL, no NON-DG
...
def main():
    ...
    # -----------------------------------------------------------------------
    # Step 1: Fetch PricingDGClassCharges
    # -----------------------------------------------------------------------
    print("\nFetching PricingDGClassCharges from Datastore...")
    query = client.query(kind="PricingDGClassCharges")
    items_raw = list(query.fetch())
    print(f"  Fetched {len(items_raw)} PricingDGClassCharges entities.")
    ...
    for e in items_raw:
        ...
        # Validate dg_class_code
        dg_class_code = str(e.get("dg_class_code", "")).strip()
        if dg_class_code not in VALID_DG_CLASS_CODES:
            skipped_dg_class.append({"pt_id": pt_id, "dg_class_code": dg_class_code})
            continue
    ...
    if skipped_dg_class:
        print(f"  Skipped (invalid dg_class_code): {len(skipped_dg_class)}")
        for s in skipped_dg_class[:10]:
            print(f"    pt_id={s['pt_id']} dg_class_code={s['dg_class_code']}")
        if len(skipped_dg_class) > 10:
            print(f"    ... and {len(skipped_dg_class) - 10} more")
    if skipped_ports:
    ...
    # -----------------------------------------------------------------------
    # Step 2: Fetch PTMonthlyRatePortCharges (kind=PT-DGC-CHARGES)
    # -----------------------------------------------------------------------
    print("\nFetching PTMonthlyRatePortCharges from Datastore...")
    query = client.query(kind="PTMonthlyRatePortCharges")
    query.add_filter("kind", "=", "PT-DGC-CHARGES")
    rates_raw = list(query.fetch())
    print(f"  Fetched {len(rates_raw)} rate entries (kind=PT-DGC-CHARGES).")
    ...
    for rate in rates_raw:
        ...
        rows_to_insert.append({
            ...
            "dg_class_code": item["dg_class_code"],
    ...
    print(f"  UOM remapped (CTR→CONTAINER, RT→W/M): {uom_remapped_count}")
    ...
    # -----------------------------------------------------------------------
    if DRY_RUN:
        print(f"\n[DRY RUN] Would insert {len(rows_to_insert)} rows into dg_class_charges.\n")
        if rows_to_insert:
            print(f"Sample (first 20 rows):")
            print(f"  {'PORT':<8} {'DIR':<8} {'STYPE':<6} {'CSIZE':<6} {'CTYPE':<6} {'DG':<6} {'CODE':<12} {'UOM':<10} {'PRICE':>10} {'COST':>10} {'EFF_FROM':<12} {'EFF_TO':<12} {'ACTIVE':<7} {'DOM':<6}")
            for r in rows_to_insert[:20]:
                print(f"  {r['port_code']:<8} {r['trade_direction']:<8} {r['shipment_type']:<6} {r['container_size']:<6} {r['container_type']:<6} {r['dg_class_code']:<6} {r['charge_code']:<12} {r['uom']:<10} {r['price']:>10.2f} {r['cost']:>10.2f} {r['effective_from']:<12} {r['effective_to']:<12} {str(r['is_active']):<7} {str(r['is_domestic']):<6}")
            if len(rows_to_insert) > 20:
                print(f"  ... and {len(rows_to_insert) - 20} more rows")
    else:
        print(f"\n[LIVE] Inserting {len(rows_to_insert)} rows into dg_class_charges...")
        inserted = 0
        skipped_conflict = 0

        insert_sql = text("""
            INSERT INTO dg_class_charges (
            ...
            ON CONFLICT ON CONSTRAINT dgc_unique DO NOTHING
        """)
        ...
        print(f"[LIVE] Inserted {inserted} rows. Skipped (conflict/duplicate): {skipped_conflict}.")
    ...
    print(f"\n{'='*60}")
    print(f"Summary:")
    print(f"  PricingDGClassCharges entities fetched: {len(items_raw)}")
    ...
    print(f"    - Invalid dg_class_code: {len(skipped_dg_class)}")
