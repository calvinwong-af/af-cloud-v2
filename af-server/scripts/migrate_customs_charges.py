"""
scripts/migrate_customs_charges.py

Migrate customs charges data from Google Cloud Datastore to PostgreSQL.

Reads:
  - PricingCustomsCharges — charge line item definitions
  - PTMonthlyRatePortCharges — monthly rate history (kind=PT-CUSTOMS-CHARGES)
...
VALID_UOMS = {"CONTAINER", "CBM", "KG", "W/M", "CW_KG", "SET", "BL"}
...
VALID_SHIPMENT_TYPES = {"FCL", "LCL", "AIR", "CB", "ALL"}
...
def main():
    ...
    # -----------------------------------------------------------------------
    # Step 1: Fetch PricingCustomsCharges
    # -----------------------------------------------------------------------
    print("\nFetching PricingCustomsCharges from Datastore...")
    query = client.query(kind="PricingCustomsCharges")
    items_raw = list(query.fetch())
    print(f"  Fetched {len(items_raw)} PricingCustomsCharges entities.")
    ...
    # -----------------------------------------------------------------------
    # Step 2: Fetch PTMonthlyRatePortCharges (kind=PT-CUSTOMS-CHARGES)
    # -----------------------------------------------------------------------
    print("\nFetching PTMonthlyRatePortCharges from Datastore...")
    query = client.query(kind="PTMonthlyRatePortCharges")
    query.add_filter("kind", "=", "PT-CUSTOMS-CHARGES")
    rates_raw = list(query.fetch())
    print(f"  Fetched {len(rates_raw)} rate entries (kind=PT-CUSTOMS-CHARGES).")
    ...
    for rate in rates_raw:
        ...
        if item["shipment_type"] not in VALID_SHIPMENT_TYPES:
            skipped_shipment_type += 1
            continue
    ...
    print(f"  UOM remapped (CTR→CONTAINER, RT→W/M, CW→CW_KG): {uom_remapped_count}")
    ...
    print(f"  Skipped (invalid shipment_type): {skipped_shipment_type}")
    ...
    # -----------------------------------------------------------------------
    if DRY_RUN:
        print(f"\n[DRY RUN] Would insert {len(rows_to_insert)} rows into customs_rates.\n")
        if rows_to_insert:
            print(f"Sample (first 20 rows):")
            print(f"  {'PORT':<8} {'DIR':<8} {'STYPE':<6} {'CODE':<12} {'UOM':<10} {'PRICE':>10} {'COST':>10} {'EFF_FROM':<12} {'EFF_TO':<12} {'ACTIVE':<7} {'DOM':<6}")
            for r in rows_to_insert[:20]:
                print(f"  {r['port_code']:<8} {r['trade_direction']:<8} {r['shipment_type']:<6} {r['charge_code']:<12} {r['uom']:<10} {r['price']:>10.2f} {r['cost']:>10.2f} {r['effective_from']:<12} {r['effective_to']:<12} {str(r['is_active']):<7} {str(r['is_domestic']):<6}")
            if len(rows_to_insert) > 20:
                print(f"  ... and {len(rows_to_insert) - 20} more rows")
    else:
        print(f"\n[LIVE] Inserting {len(rows_to_insert)} rows into customs_rates...")
        inserted = 0
        skipped_conflict = 0
        ...
        insert_sql = text("""
            ...
            ON CONFLICT ON CONSTRAINT customs_rates_unique DO NOTHING
        """)
        ...
        print(f"[LIVE] Inserted {inserted} rows. Skipped (conflict/duplicate): {skipped_conflict}. Skipped (validation): {len(skipped_uom) + skipped_no_item + skipped_month + skipped_direction + skipped_shipment_type}.")
    ...
    print(f"\n{'='*60}")
    print(f"Summary:")
    print(f"  PricingCustomsCharges entities fetched: {len(items_raw)}")
    ...
    print(f"    - Invalid shipment_type: {skipped_shipment_type}")
    print(f"{'='*60}")
