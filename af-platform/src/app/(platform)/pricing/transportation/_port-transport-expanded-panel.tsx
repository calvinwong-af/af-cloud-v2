...
interface PortTransportExpandedPanelProps {
  detail: PortTransportRateCard;
  months: MonthBucket[];
...
// Adapt PortTransportRate to be compatible with CostSparkline's RateDetail expectations
type SparklineRate = {
  id: number;
...
export function PortTransportExpandedPanel({
  detail,
  months,
  companiesMap,
  totalWidth,
  ...
  companiesList,
  onRatesChanged,
}: PortTransportExpandedPanelProps) {
  ...
  const handleTerminate = async () => {
    ...
    setSaving(true);
    try {
      await updatePortTransportRateAction(panelMode.rateId, { effective_to: formTerminateDate });
  ...
  const handleDelete = async (rateId: number) => {
    setSaving(true);
    try {
      await deletePortTransportRateAction(rateId);
  ...
  const getLatestEffective = (rates: PortTransportRate[]): string | null => {
    return rates.map(r => r.effective_from).filter(Boolean).sort().at(-1) ?? null;
  };

  const formatRatesRange = (rates: PortTransportRate[]): string => {
  ...
  const getEffectiveRate = (rates: PortTransportRate[]): PortTransportRate | undefined => {
  ...
  const buildSurchargesMap = (rates: PortTransportRate[]): Map<string, SurchargeItem[] | null> => {
    const sorted = [...rates].sort(
      (a, b) => (b.effective_from ?? '').localeCompare(a.effective_from ?? ''),
    ) as SparklineRate[];
  ...
  const priceRefRates = (detail.rates_by_supplier?.['null'] ?? []) as PortTransportRate[];
  ...
  const supplierRows = supplierEntries
    ...
    .sort(([idA, ratesA], [idB, ratesB]) => {
      const latestA = getLatestEffective(ratesA as PortTransportRate[]) ?? '';
      const latestB = getLatestEffective(ratesB as PortTransportRate[]) ?? '';
    ...
    <>
      <div>
        ...
        {supplierRows.map(([supplierId, rawRates]) => {
          const rates = rawRates as PortTransportRate[];
