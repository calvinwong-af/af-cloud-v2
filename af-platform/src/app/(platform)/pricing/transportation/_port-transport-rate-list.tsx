...
import { PortTransportExpandedPanel } from './_port-transport-expanded-panel';
      ...
      <div className="overflow-auto flex-1">
        <div style={{ minWidth: `${totalWidth}px` }}>
          {cards.map(card => {
            ...
            // Full label for tooltip: "area_name, state_name" (or just area_name if no state)
            const areaFullLabel = stateName ? `${areaName}, ${stateName}` : areaName;
            // Truncated display label (max 32 chars)
                    ...
                    {!expandedDetail ? (
                      ...
                      <div style={{ maxHeight: '320px', overflowY: 'auto' }}>
                        <PortTransportExpandedPanel
                          ...
                          onRatesChanged={() => {
                            onCardsRefresh();
                            fetchPortTransportRateCardDetailAction(card.id).then(r => {
                              if (r?.success) setExpandedDetail(r.data);
                            });
                          }}
                        />
