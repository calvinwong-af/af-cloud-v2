...
export function HaulageRateCardsTab({ countryCode, alertFilter }: { countryCode?: string; alertFilter?: string }) {
  ...
  const [containerSizeFilter, setContainerSizeFilter] = useState('');
  ...
  useEffect(() => {
    ...
    fetchHaulageContainerSizesAction().then(r => {
      if (r?.success) setContainerSizes(r.data);
    });
  }, []);
  ...
  useEffect(() => {
    ...
    fetchHaulagePortsAction(country || undefined).then(r => {
  ...
  useEffect(() => {
    if (portFilter) {
      fetchHaulageAreasAction(portFilter).then(r => {
  ...
  const fetchCards = useCallback(() => {
    ...
    fetchHaulageRateCardsAction({
      ...
      containerSize: containerSizeFilter || undefined,
  ...
  }, [country, portFilter, areaFilter, containerSizeFilter, showInactive, showIssuesOnly]);
  ...
  const containerSizeOptions = [
    ...
    ...containerSizes.map(cs => ({ value: cs.container_size, label: cs.label })),
  ];
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-center gap-3">
        ...
        <div className="w-40">
          <PortCombobox value={containerSizeFilter} onChange={setContainerSizeFilter} options={containerSizeOptions} placeholder="All Sizes" />
        </div>
      ...
      <div className="relative">
        <input
          ...
          placeholder="Filter by area, container size, rate card key…"
      ...
      {!portFilter && !showIssuesOnly ? (
        <div className="bg-white rounded-xl border border-[var(--border)] py-16 flex flex-col items-center gap-3 text-[var(--text-muted)]">
          ...
          <p className="text-sm">Select a port to view haulage rate cards</p>
        </div>
      ) : (
        <>
          <HaulageTimeSeriesRateList
            cards={pagedCards}
            loading={loading}
            portsMap={portsMap}
