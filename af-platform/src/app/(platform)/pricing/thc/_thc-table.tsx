...
export function THCTable() {
  const [rates, setRates] = useState<THCRate[]>([]);
  ...
  const [editRate, setEditRate] = useState<THCRate | null>(null);
  ...
  const handleSave = async (data: Omit<THCRate, 'id' | 'created_at' | 'updated_at'>) => {
    if (editRate) {
      await updateTHCRateAction(editRate.id, data);
    } else {
      await createTHCRateAction(data);
    }
    fetchRates();
  };

  const handleDelete = async (id: number) => {
    await deleteTHCRateAction(id);
    fetchRates();
  };
    ...
    <div className="space-y-4">
      ...
      <div className="relative">
        <input
          ...
          placeholder="Filter by port, charge code, description, equipment..."
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-[var(--border)] bg-[var(--surface)]">
                ...
                <th className="text-left px-3 py-2 text-xs font-medium text-[var(--text-muted)]">Equipment</th>
            ...
            <tbody>
              {loading ? (
                ...
                <tr><td colSpan={10} className="px-3 py-8 text-center text-[var(--text-muted)]">No rates found</td></tr>
                ...
                <tr key={rate.id} className="border-b border-[var(--border)] last:border-0 hover:bg-[var(--surface)]/50">
                  ...
                  <td className="px-3 py-2 text-xs text-[var(--text-muted)]">{rate.equipment_type ?? '\u2014'}</td>
