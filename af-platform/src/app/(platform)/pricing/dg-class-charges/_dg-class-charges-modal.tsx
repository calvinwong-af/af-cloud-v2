...
const UOMS = ['CONTAINER', 'CBM', 'KG', 'W/M', 'SET', 'BL'] as const;
const UOM_DISPLAY: Record<string, string> = { CONTAINER: 'CTR' };
const DG_CLASS_CODES = ['DG-2', 'DG-3'] as const;

// ---------------------------------------------------------------------------
// Exported types
// ---------------------------------------------------------------------------

export interface DgClassChargeModalSeed {
  card_id: number;
  card_key: string;
  port_code: string;
...
interface DgClassChargeRateUpdatePayload {
  price?: number; cost?: number; effective_from?: string; effective_to?: string | null;
}
...
export type DgClassChargeModalPayload =
  | { mode: 'new'; data: DgClassChargeCreatePayload }
  | { mode: 'edit-rate'; rateId: number; data: DgClassChargeRateUpdatePayload }
  | { mode: 'edit-card'; cardId: number; data: DgClassChargeCardUpdatePayload };

// ---------------------------------------------------------------------------
// Props
// ---------------------------------------------------------------------------

interface DgClassChargesModalProps {
  open: boolean;
  onClose: () => void;
  onSave: (payload: DgClassChargeModalPayload) => Promise<void>;
  onDelete?: () => Promise<void>;
  mode: 'new' | 'edit-rate' | 'edit-card';
  seed?: DgClassChargeModalSeed;
  portOptions?: { value: string; label: string; sublabel?: string }[];
}

// ---------------------------------------------------------------------------
// Badge helpers for edit-rate header
// ---------------------------------------------------------------------------

const dirBadge = (d: string) => {
  const cls = d === 'IMPORT' ? 'bg-blue-50 text-blue-700' : 'bg-emerald-50 text-emerald-700';
  return <span className={`text-[10px] font-medium px-1.5 py-0.5 rounded ${cls}`}>{d === 'IMPORT' ? 'IMP' : 'EXP'}</span>;
};
const modalTypeBadge = (t: string) => {
  const colors: Record<string, string> = { FCL: 'bg-sky-50 text-sky-700', LCL: 'bg-violet-50 text-violet-700', AIR: 'bg-orange-50 text-orange-700' };
  return <span className={`text-[10px] font-medium px-1.5 py-0.5 rounded ${colors[t] ?? 'bg-slate-100 text-slate-600'}`}>{t}</span>;
};
const dgBadge = (dg: string) => {
...
const containerBadge = (size: string, type: string) => {
  const label = (size === 'ALL' && type === 'ALL') ? 'ALL' : `${size}/${type}`;
  return <span className="text-[10px] font-medium px-1.5 py-0.5 rounded bg-slate-100 text-slate-600">{label}</span>;
};
...
export function DgClassChargesModal({ open, onClose, onSave, onDelete, mode, seed, portOptions = [] }: DgClassChargesModalProps) {
  // Card fields
  ...
  const [dgClassCode, setDgClassCode] = useState<string>('DG-2');
  ...
  const title = mode === 'new'
    ? (seed ? `New Rate \u2014 ${seed.charge_code}` : 'Add DG Class Charge')
    : mode === 'edit-rate'
      ...
      if (mode === 'new') {
        await onSave({
          ...
          data: {
            ...
            dg_class_code: (seed?.dg_class_code ?? dgClassCode) as 'DG-2' | 'DG-3',
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
      <div className="bg-white rounded-xl border border-[var(--border)] shadow-xl w-full max-w-lg mx-4 max-h-[90vh] overflow-y-auto">
        ...
        <div className="p-4 space-y-3">
          ...
          {mode === 'edit-rate' && seed && (
            <div className="px-4 pt-3 pb-2 rounded-lg bg-[var(--surface)] border border-[var(--border)] mb-3">
              <div className="flex items-center gap-1.5">
                <span className="text-sm font-semibold text-[var(--text)]">{seed.charge_code}</span>
                <span className="text-sm text-[var(--text-muted)]">&mdash; {seed.description}</span>
              </div>
              <div className="flex items-center gap-1.5 mt-1 flex-wrap">
                {dirBadge(seed.trade_direction)}
                {modalTypeBadge(seed.shipment_type)}
                {dgBadge(seed.dg_class_code)}
                {containerBadge(seed.container_size, seed.container_type)}
                <span className="text-[11px] text-[var(--text-muted)]">{seed.port_code}</span>
              </div>
            </div>
          )}
            ...
            <>
              ...
              <label className="space-y-1 block">
                ...
                <input value={chargeCode} onChange={e => setChargeCode(e.target.value.toUpperCase())}
                  className={inputCls} placeholder="DG-INSP" />
              </label>
              <label className="space-y-1 block">
                <span className="text-xs font-medium text-[var(--text-muted)]">Description</span>
                <input value={description} onChange={e => setDescription(e.target.value)}
                  className={inputCls} placeholder="DG inspection fee" />
              </label>
            </>
          )}
