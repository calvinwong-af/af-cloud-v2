/**
 * /orders/deliveries — Ground Transport Orders list
 *
 * Haulage and trucking orders.
 * Moved from /ground-transport.
 */
              ...
              <tbody>
                {orders.map((order) => {
                  ...
                  return (
                    <tr
                      key={order.order_id}
                      onClick={() => window.open(`/orders/deliveries/${order.order_id}`, '_blank')}
                    ...
                    >
                      ...
                      <td className="px-4 py-3 text-xs text-[var(--text-mid)] max-w-[200px] truncate">
                        {firstStop ? `${originLabel} → ${destLabel}` : '—'}
                      </td>
                      <td className="px-4 py-3">
...
export default function DeliveriesPage() {
  return <Suspense><DeliveriesPageInner /></Suspense>;
}
